...
const getInitialCode = async (componentName, example) => {
  const codeExample = await import(
    `!!raw-loader!../../examples/${componentName
      .split(/(?=[A-Z])/)
      .join("-")
      .toLowerCase()}/${example}.jsx`
  );
  if (!codeExample.default || typeof codeExample.default !== "string")
    return "";
  ...
  return (
    code && (
      <Box maxWidth={CODE_WIDTH} m="0 auto">
        ...
        <CodeButton onClick={() => setShowCode(!showCode)}>
